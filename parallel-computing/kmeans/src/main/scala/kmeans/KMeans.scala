...
class KMeans extends KMeansInterface {
  ...
  def classify(points: Seq[Point], means: Seq[Point]): Map[Point, Seq[Point]] = {
    ...
    means.map(m => (m, groups.getOrElse(m, Seq()))).toMap
  }

  def classify(points: ParSeq[Point], means: ParSeq[Point]): ParMap[Point, ParSeq[Point]] = {
    val groups = points.groupBy( p => findClosest(p, means))
    means.map(m => (m, groups.getOrElse(m, ParSeq()))).toMap
  }
  ...
  def update(classified: Map[Point, Seq[Point]], oldMeans: Seq[Point]): Seq[Point] = {
    oldMeans.iterator.map( p => findAverage(p, classified.getOrElse(p, Seq()))).toList
  }
  ...
  def converged(eta: Double, oldMeans: Seq[Point], newMeans: Seq[Point]): Boolean = {
    oldMeans match {
      case Nil => true
      case _ => if (oldMeans.head.squareDistance(newMeans.head) <= eta)
  ...
  def converged(eta: Double, oldMeans: ParSeq[Point], newMeans: ParSeq[Point]): Boolean = {
    ...
    else if (oldMeans.head.squareDistance(newMeans.head) <= eta)
      converged(eta, oldMeans.tail, newMeans.tail)
    else
      false
  }
