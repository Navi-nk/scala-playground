...
class HuffmanSuite {
  ...
  @Test def `times test`: Unit =
    assertEquals(List(('a', 3), ('b', 2)), times(List('a', 'b', 'a', 'b', 'a')))
  ...
  @Test def `combine of some leaf list by preseving order (15pts)`: Unit = {
  ...
  @Test def `until test`: Unit = {
    ...
    assertEquals(List(Fork(Leaf('z', 7), Fork(Leaf('x', 4), Fork(Leaf('e', 2), Leaf('t', 3), List('e', 't'), 5), List('x', 'e', 't'), 9), List('z', 'x', 'e', 't'), 16)), until(singleton, combine)(leaflist))
  }
  ...
  @Test def `code table create test`: Unit = {
    val tree = Fork(Leaf('z', 7), Fork(Leaf('x', 4), Fork(Leaf('e', 2), Leaf('t', 3), List('e', 't'), 5), List('x', 'e', 't'), 9), List('z', 'x', 'e', 't'), 16)
    assertEquals(List(('z',List(0)), ('x',List(1, 0)), ('e',List(1, 1, 0)), ('t',List(1, 1, 1)))
      , convert(tree))
