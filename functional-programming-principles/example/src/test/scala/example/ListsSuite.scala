 ...
 */
 class ListsSuite {
  ...
  @Test def `sum of negative numbers` : Unit = {
    assert(sum(List(-1,-10,-25,-100)) == -136)
  }

  @Test def `sum of positive and negative numbers` : Unit = {
    assert(sum(List(-100,23,0,-200,500)) == 223)
  }
  ...
  @Test def `sum of repeated numbers in list` : Unit = {
    assert(sum(List(10,10,10,10,10,10,10,10)) == 80)
  }
  ...
  @Test def `max of all negatives`: Unit = {
    assert(max(List(-30, -100, -1000000, -1)) == -1)
  }

  @Test def `max of positive and negative numbers`: Unit = {
    assert(max(List(-30, 56,-100, 0,-1000000, -1)) == 56)
  }

  @Test def `max of repeated numbers`: Unit = {
    assert(max(List(-69, -69, -69, -69)) == -69)
  }
  ...
  @Test def `max of empty list`: Unit = {
    try{
      ...
      case ex: Exception => Assert.fail("Undesired Exception thrown")
    }
  }
