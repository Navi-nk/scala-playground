...
object TimeUsage extends TimeUsageInterface {
  ...
  /** @return An RDD Row compatible with the schema produced by `dfSchema`
    ...
    */
  def row(line: List[String]): Row = Row.fromSeq(line.head :: line.tail.map(item => if (item == null) 0D else item.toDouble))
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    val t1 = new ArrayBuffer[Column]
    val t2 = new ArrayBuffer[Column]
    val t3 = new ArrayBuffer[Column]
    columnNames.foreach( c => {
      if(PRIMARY.exists(c.startsWith))
        t1.append(col(c))
      else if (WORK.exists(c.startsWith))
        t2.append(col(c))
      else if (OTHERS.exists(c.startsWith))
        t3.append(col(c))
    })
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    summed.groupByKey(row => (row.working, row.sex, row.age))
      ...
      .map{case ((working, sex, age), primary, work ,others) => TimeUsageRow(working, sex, age, primary, work ,others)}
