...
trait DecoderInstances {
  ...
  implicit def listDecoder[A](implicit decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromFunction {
      case Json.Arr(a) => Some(a.flatMap( b => decoder.decode(b)))
      case _ => throw new Error
    }
...
trait ContactsCodecs {
  ...
  implicit def contactsEncoder(implicit encoder: Encoder[Person]): Encoder[Contacts] =
    ObjectEncoder.fromFunction(
      c => Json.Obj(Map("people" -> Json.Arr(c.people.map(encoder.encode))))
    )
