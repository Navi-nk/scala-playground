...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] = {
    for {
      legal <- b.legalNeighbors
    } yield (legal._1, legal._2 :: history)
  }.to(LazyList)
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    if(initial.isEmpty) LazyList.empty
      ...
      val more: LazyList[(Block, List[Move])] = for {
        initMove <- initial
        next <- newNeighborsOnly(neighborsWithHistory(initMove._1, initMove._2), explored)
      } yield next
      val exploredBlock = more.map((b) => b._1)
      initial ++ from(more, explored ++ exploredBlock)
    }
  }
  ...
  lazy val pathsToGoal: LazyList[(Block, List[Move])] =   for {
    ...
    if paths._1 equals Block(goal,goal)
  } yield paths
  ...
  lazy val solution: List[Move] = {
   findOptimalSolution(pathsToGoal.map(_._2).toList).reverse
  }

  def findOptimalSolution(paths: List[List[Move]]): List[Move] = {
    paths match {
      case Nil => Nil
      case x :: Nil => x
      case x :: y :: rest => findOptimalSolution( (if(x.length < y.length) x else y) :: rest )
    }
  }
}
